-- Script para reestructurar la base de datos para el sistema de reservas con cuotas
-- Fecha: 2025-08-26
-- Descripción: Renombrar tablas y actualizar relaciones para manejar reservas con cuotas y pagos completos

USE jaguar_expeditions;

-- Primero, verificar que no haya datos críticos antes de proceder
...
-- Renombrar clientes_reserva a reserva_cuotas (para reservas con pago en cuotas)
RENAME TABLE clientes_reserva TO reserva_cuotas;

-- Renombrar reservas a reservas_pago (para reservas con pago completo)
RENAME TABLE reservas TO reservas_pago;

-- =============================================
-- PASO 2: MODIFICAR TABLA reserva_cuotas
-- =============================================

-- Agregar campos necesarios para el sistema de cuotas
ALTER TABLE reserva_cuotas 
ADD COLUMN tour_id INT(11) NOT NULL AFTER id,
ADD COLUMN codigo_reserva VARCHAR(20) UNIQUE NOT NULL AFTER tour_id,
...
-- =============================================
-- PASO 3: MODIFICAR TABLA pagos PARA SOPORTAR AMBOS TIPOS
-- =============================================
...
-- =============================================
-- PASO 4: CREAR TRIGGER PARA CALCULAR CUOTAS AUTOMÁTICAMENTE
-- =============================================
...
BEGIN
    ...
    IF NEW.codigo_reserva IS NULL OR NEW.codigo_reserva = '' THEN
        SET NEW.codigo_reserva = CONCAT('RC-', DATE_FORMAT(NOW(), '%Y%m%d'), '-', LPAD(NEW.id, 4, '0'));
    END IF;
END//
...
-- =============================================
-- PASO 5: CREAR VISTAS PARA CONSULTAS SIMPLIFICADAS
-- =============================================

-- Vista para todas las reservas (cuotas + pago completo)
CREATE OR REPLACE VIEW vista_todas_reservas AS
SELECT 
